...
export default class WorkoutPage {
	...
	async addExercise({ exercise, sets, repetitions, rest, notes }: WorkoutExercise) {
		...
		await exerciseDialog.getByLabel(m.sets()).fill(sets.toString());
		await exerciseDialog.getByLabel(m.repetitions()).fill(repetitions.toString());
		await exerciseDialog.getByLabel(m.rest()).fill(rest.toString());
